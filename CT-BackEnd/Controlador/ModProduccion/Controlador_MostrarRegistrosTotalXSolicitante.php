...
if(isset($_POST['_fInicial']) && isset($_POST['_fFinal'])){
    ...
    $solicitante = $_POST['_solicitante'];
    $fInicial = $_POST['_fInicial'];
    $fFinal = $_POST['_fFinal'];
    
    if($Model_Produccion->mostrarRegistrosTotalXSolicitante($solicitante,$fInicial,$fFinal)){
        
        $data = $Model_Produccion->mostrarRegistrosTotalXSolicitante($solicitante,$fInicial,$fFinal);
